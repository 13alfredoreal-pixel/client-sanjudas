...
export const ReviewsSection = ({ bookId, user }) => {
    const {
        reviews, averageRating, totalReviews, loading, newReview, setNewReview,
        isSubmitting, handleAddReview, handleDeleteReview, userHasReviewed
    ...
    return (
        <div className="animate-fade-in-up mt-8 pb-12">
            <div className="flex items-center gap-4 mb-8">
                <h3 className="text-[1.4rem] font-extrabold text-slate-50 m-0">
            ...
            <div className="grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-8">
                ...
                <div className="flex flex-col gap-5">
                    {reviews.length === 0 ? (
                        <p className="text-slate-500 italic">Aún no hay reseñas. ¡Sé el primero en calificar!</p>
                    ) : (
                        reviews.map(review => (
                            <div key={review._id} className="bg-slate-900/40 backdrop-blur-[20px] saturate-[180%] border border-white/10 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.1),0_10px_15px_-3px_rgba(0,0,0,0.2),inset_0_0_0_1px_rgba(255,255,255,0.05)] p-5 rounded-2xl relative">
                                <div className="flex gap-4 items-start">
                                    ...
                                    {(user?.role === 'ADMIN_ROLE' || review.user?.uid === user?.uid || review.user?._id === user?.uid) && (
                                        <button
