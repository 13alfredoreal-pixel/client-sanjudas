...
import { getBookById, getReviewsService, addReviewService, deleteReviewService, getSignedPdfUrl } from '../services/apiService';
...
export const useBookViewer = (id, user) => {
        ...
        if (data.error) {
            ...
            if (!revData.error) {
                ...
                if (user) {
                    ...
                    const existing = (revData.reviews || []).find(r => (r.user?._id === uid || r.user === uid) && uid);
    ...
    useEffect(() => {
        const handleKeyDown = (e) => {
            ...
            if (isCtrl && (key === 's' || key === 'p' || key === 'u' || key === 'i' || key === 't' || key === 'n')) {
                e.preventDefault();
