...
export const BookViewerPage = ({ user }) => {
    const { id } = useParams();
    const {
    book, loading, error, numPages, pageNumber, theme, setTheme,
    scale, setScale, isMobile, showReviews, setShowReviews,
    reviews, userRating, setUserRating, userComment, setUserComment,
    submittingReview, hasReviewed, handleSubmitReview, handleDeleteReview,
    onDocumentLoadSuccess, changePage, getFilter, getBackground, isAdmin, navigate,
    averageRating, pdfUrl
    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center min-h-[80vh] flex-col gap-6">
                ...
                <p className="text-slate-400 font-semibold tracking-widest animate-pulse">SINCRONIZANDO BIBLIOTECA...</p>
                <style>{`
    ...
    if (error || !book) {
        return (
            <div className="text-center p-20 text-slate-400">
                ...
                <button onClick={() => navigate('/')} className="bg-white/5 border border-white/10 rounded-2xl py-2.5 px-6 text-slate-100 cursor-pointer font-bold transition-all duration-200 text-sm hover:bg-white/10">Regresar a la Biblioteca</button>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen flex flex-col gap-6 p-6 bg-slate-950 overflow-hidden">
            ...
            <div className="glass-effect py-5 px-10 rounded-[2rem] flex items-center gap-8 border border-white/10 shadow-[0_10px_30px_rgba(0,0,0,0.5)] relative z-30">
                <button onClick={() => navigate('/')} className="bg-white/5 border border-white/10 rounded-2xl py-2.5 px-6 text-slate-100 cursor-pointer font-bold transition-all duration-200 text-sm hover:bg-white/10">← Salir</button>

                <div className="flex-1">
                    <h1 className="m-0 text-[1.4rem] font-black text-slate-100 tracking-tight">{book.title}</h1>
                    <p className="m-0 text-blue-400 text-[0.85rem] font-bold uppercase tracking-wide">
                        Páginas {pageNumber} {!isMobile && pageNumber < numPages && `& ${pageNumber + 1}`} de {numPages}
                    </p>
                            ...
                            {!hasReviewed ? (
                                <div className="flex items-center gap-4 bg-white/5 px-4 py-2 rounded-2xl border border-white/10 shadow-lg backdrop-blur-sm group transition-all hover:border-blue-500/30">
                                    <div className="flex flex-col">
                                        <span className="text-[9px] text-blue-400 font-black uppercase tracking-[0.2em] leading-none mb-1">Tu opinión importa</span>
                                    ...
                                    >
                                        <span className="text-white font-black text-sm group-hover/btn:text-blue-400 transition-colors">{reviews.length}</span>
                                        <span className="text-[8px] text-slate-500 font-bold uppercase tracking-tighter group-hover/btn:text-slate-300">Reseñas</span>
                                    </button>
                                </div>
                            ) : (
                                <button 
                                ...
                                >
                                    <div className="flex flex-col items-start">
                                        <span className="text-[9px] text-amber-400 font-black uppercase tracking-[0.2em] mb-1">Promedio General</span>
                                        <div className="flex gap-0.5">
                                            {[1, 2, 3, 4, 5].map(star => (
                                                <FaStar 
                                                    key={star} 
                                                    className={star <= Math.round(averageRating) ? "text-amber-400" : "text-slate-700"} 
                                                    size={12}
                                                />
                                            ))}
                                        </div>
                                    ...
                                    <div className="flex flex-col items-center">
                                        ...
                                        <span className="text-[8px] text-slate-500 font-bold uppercase tracking-tighter">{reviews.length} Reseñas</span>
                                    </div>
                                </button>
                            )}
                        </div>
                ...
                <div className="flex gap-3 bg-black/30 p-1.5 rounded-xl border border-white/5">
                    <ThemeBtn active={theme === 'default'} onClick={() => setTheme('default')} color="#3b82f6" title="Vista Normal" />
                    <ThemeBtn active={theme === 'sepia'} onClick={() => setTheme('sepia')} color="#d4a373" title="Modo Lectura Sepia" />
                    <ThemeBtn active={theme === 'night'} onClick={() => setTheme('night')} color="#1e293b" title="Modo Nocturno" />
                </div>

                <div className="flex gap-2">
                    <button onClick={() => setScale(s => Math.max(0.4, s - 0.1))} className="bg-white/5 border border-white/10 rounded-2xl py-1.5 px-5 text-slate-100 cursor-pointer font-bold transition-all duration-200 text-xl flex items-center justify-center hover:bg-white/10">-</button>
                    <button onClick={() => setScale(s => Math.min(2.5, s + 0.1))} className="bg-white/5 border border-white/10 rounded-2xl py-1.5 px-5 text-slate-100 cursor-pointer font-bold transition-all duration-200 text-xl flex items-center justify-center hover:bg-white/10">+</button>
                </div>
            </div>
            ...
            <div className={`fixed top-0 right-0 h-full w-[450px] max-w-full bg-slate-900/95 backdrop-blur-2xl border-l border-white/10 z-50 shadow-[-20px_0_50px_rgba(0,0,0,0.5)] transition-transform duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] flex flex-col ${showReviews ? 'translate-x-0' : 'translate-x-[105%]'}`}>
                ...
                <div className="flex-1 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                    {/* POMULARIO DE RESEÑA */}
                    {user && !hasReviewed && (
                        <form onSubmit={handleSubmitReview} className="mb-8 bg-white/5 p-4 rounded-2xl border border-white/5">
                            ...
                            <div className="flex flex-col gap-2">
                                <button disabled={submittingReview} type="submit" className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-2.5 rounded-xl transition-colors disabled:opacity-50 shadow-lg shadow-blue-500/20">
                    ...
                    <div className="flex flex-col gap-4">
                        {reviews.length === 0 ? (
                            <p className="text-slate-500 text-center font-medium my-10">Sé el primero en calificar este libro.</p>
                        ) : (
                            reviews.map(review => (
                                <div key={review._id} className="bg-white/5 p-4 rounded-2xl flex gap-4 border border-white/5 relative group">
                                    <img src={review.user?.profilePicture} alt={review.user?.name} className="w-10 h-10 rounded-full object-cover border border-white/10 shadow-sm" />
                                    <div className="flex-1">
                                        <div className="flex justify-between items-start mb-1">
                                            <p className="m-0 font-bold text-slate-200 text-sm">{review.user?.name} {review.user?.surname}</p>
                                            <div className="flex gap-0.5 mt-0.5">
                                                {[1, 2, 3, 4, 5].map(s => (
                                                    <FaStar key={s} size={10} color={s <= review.rating ? '#fbbf24' : '#334155'} />
                                                ))}
                                            </div>
                                        </div>
                                        <p className="m-0 text-slate-400 text-xs leading-relaxed">{review.comment}</p>
                                        <p className="m-0 text-slate-600 text-[10px] mt-2 font-medium">{new Date(review.createdAt).toLocaleDateString()}</p>
                                    </div>
