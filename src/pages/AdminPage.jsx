        ...
        <div className="min-h-screen p-8 box-border max-w-[1100px] mx-auto">
            ...
            <div className="animate-fade-in-up mb-8">
                <div className="flex items-center gap-4 mb-2">
                    <button onClick={() => navigate('/')} className="bg-white/5 border border-white/10 rounded-xl px-4 py-1.5 text-slate-400 cursor-pointer text-sm font-inherit hover:bg-white/10 transition-colors">← Catálogo</button>
                ...
                <p className="text-slate-500 m-0">
                    Gestiona los libros de la biblioteca digital · {books.length} libro{books.length !== 1 ? 's' : ''} registrado{books.length !== 1 ? 's' : ''}
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[minmax(300px,400px)_1fr] gap-8 items-start">
                    ...
                    <div>
                        ...
                        <div className="flex flex-col gap-3">
                            {analytics?.mostReviewedBooks?.map(item => (
                                <div key={item._id} className="flex items-center gap-3 bg-white/5 p-3 rounded-2xl border border-white/5">
                                    <div className="w-10 h-12 bg-slate-800 rounded flex items-center justify-center text-xs">📚</div>
                                    <div className="flex-1 min-w-0">
                                        <p className="m-0 text-slate-200 font-bold text-sm truncate">{item.bookDetails.title}</p>
                    ...
                    <div>
                        ...
                        <div className="flex flex-col gap-2 max-h-[200px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-white/10">
                            {analytics?.recentAuditLogs?.map(log => (
                                <div key={log._id} className="text-[11px] bg-black/20 p-2 rounded-xl border border-white/5">
                                    <span className="text-purple-400 font-bold mr-2">{log.action}</span>
                                    <span className="text-slate-400">{log.details ? (typeof log.details === 'string' ? log.details : JSON.stringify(log.details)) : ''}</span>
                                    <div className="text-slate-600 mt-1">{new Date(log.createdAt).toLocaleString()}</div>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="glass-premium rounded-3xl p-7 col-span-full mb-4">
                    ...
                    <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4">
                        {allUsers.filter(u => u.role !== 'ADMIN_ROLE').map(u => (
                            <div key={u._id || u.uid} className="glass-effect rounded-2xl p-4 flex items-center gap-4">
                                <img
                                    src={getImageUrl(u.profilePicture) || `https://ui-avatars.com/api/?name=${u.username}&background=random`}
                                    alt={u.username}
                                    onError={(e) => { e.target.onerror = null; e.target.src = `https://ui-avatars.com/api/?name=${u.username}&background=random`; }}
                                    className="w-10 h-10 rounded-full object-cover border border-white/10"
                                />
                                <div className="flex-1">
                                    <p className="m-0 font-bold text-slate-50 text-[0.9rem]">{u.username}</p>
                                    <p className="m-0 text-xs text-slate-400">{u.name} {u.surname}</p>
                                </div>
                                <button
                ...
                <div className="glass-premium rounded-3xl p-7 col-span-full mb-4">
                    ...
                    <form onSubmit={handleCreateCategory} className="flex flex-col gap-4 mb-8">
                        ...
                        <div className="bg-white/5 border border-white/5 rounded-2xl p-4">
                            <p className="text-[0.65rem] text-slate-500 font-bold uppercase tracking-wider m-0 mb-3">Sugerencias de Materias:</p>
                    ...
                    <div className="flex flex-wrap gap-3">
                        {categories.map(c => (
                            <div key={c._id} className="bg-white/5 border border-white/10 rounded-xl pl-4 pr-1 py-1 flex items-center gap-3">
                                <span className="text-slate-50 font-semibold text-sm">{c.icon} {c.name}</span>
                                <button
                        ...
                        {categories.length === 0 && <span className="text-slate-400 text-sm italic">No hay categorías. Crea una.</span>}
                    </div>
                </div>
                ...
                <div className="glass-premium rounded-3xl p-7">
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                        ...
                        <div>
                            <label className="block mb-1 text-[0.78rem] font-semibold text-slate-400 tracking-[0.04em]">Título *</label>
                            <input name="title" value={form.title} onChange={handleFormChange} placeholder="Título del libro" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-slate-50 text-sm outline-none box-border focus:border-purple-400 transition-colors" required />
                        </div>

                        <div>
                            <label className="block mb-1 text-[0.78rem] font-semibold text-slate-400 tracking-[0.04em]">Autor *</label>
                            <input name="author" value={form.author} onChange={handleFormChange} placeholder="Nombre del autor" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-slate-50 text-sm outline-none box-border focus:border-purple-400 transition-colors" required />
                        </div>

                        <div>
                            <label className="block mb-1 text-[0.78rem] font-semibold text-slate-400 tracking-[0.04em]">Materia (Categoría)</label>
                            <select name="category" value={form.category} onChange={handleFormChange} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-slate-50 text-sm outline-none box-border cursor-pointer focus:border-purple-400 transition-colors" required>
                                <option value="" disabled className="bg-slate-900">Selecciona una categoría</option>
                                {categories.map(c => (
                                ...
                                {!categories.some(c => c.name === 'Otros') && (
                                    <option value="Otros" className="bg-slate-900 text-slate-50">Otros</option>
                                )}
                            </select>
                        </div>

                        <div>
                            <label className="block mb-1 text-[0.78rem] font-semibold text-slate-400 tracking-[0.04em]">Descripción</label>
                            <textarea
                                name="description"
                        ...
                        <div>
                            <label className="block mb-1 text-[0.78rem] font-semibold text-slate-400 tracking-[0.04em]">Archivo PDF * (máx. 50 MB)</label>
                            <input
                                id="pdf-input"
                                type="file"
                            ...
                            />
                            {pdfFile && <p className="mt-1 mb-0 text-[0.72rem] text-blue-400">✓ {pdfFile.name}</p>}
                        </div>

                        <div>
                            <label className="block mb-1 text-[0.78rem] font-semibold text-slate-400 tracking-[0.04em]">Portada (opcional, imagen)</label>
                            <input
                                id="cover-input"
                                type="file"
                                accept="image/*"
                                ...
                                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-slate-50 text-sm outline-none box-border focus:border-purple-400 transition-colors"
                            />
                            {coverFile && <p className="mt-1 mb-0 text-[0.72rem] text-blue-400">✓ {coverFile.name}</p>}
                        </div>
                ...
                {/* LISTA DE LIBROS */}
                <div>
                    <h2 className="m-0 mb-5 text-lg text-slate-50 font-bold">
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col gap-3">
                            {books.map(book => (
                                <div key={book._id} className="glass-effect rounded-2xl py-4 px-5 flex items-center gap-4">
                                    {/* Portada miniatura */}
                                    <div 
                                        className="w-12 h-16 rounded-lg shrink-0 flex items-center justify-center text-xl"
                                        style={{ background: book.coverUrl ? `url(${book.coverUrl}) center/cover` : 'linear-gradient(135deg, #3b82f6, #8b5cf6)' }}
                                    >
                                        {!book.coverUrl && '📘'}
                                    </div>
                                    ...
                                    <div className="flex gap-2 shrink-0">
                                        <a href={getPdfUrl(book.pdfUrl)} target="_blank" rel="noopener noreferrer" className="bg-blue-500/10 border border-blue-500/30 text-blue-300 rounded-xl px-3 py-1.5 no-underline text-xs font-semibold whitespace-nowrap hover:bg-blue-500/20 transition-colors">Ver PDF</a>
                                        <button
...
const StatCard = ({ icon, title, value, color, textColor }) => (
    <div className={`glass-premium p-6 rounded-3xl flex items-center gap-5 border border-white/5 bg-gradient-to-br ${color}`}>
        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl bg-black/20 ${textColor} border border-white/5`}>
            {icon}
        </div>
        <div>
